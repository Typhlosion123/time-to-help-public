  ...
  if (user && user.emailVerified) {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // --- Re-check current tab (Fixes race condition) ---
...
async function handleTabChange(tabId) {
  // First, log time for the tab we're leaving
  await logTime();
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === SYNC_ALARM_NAME && currentUserId) {
    ...
    try {
      ...
      if (localData) {
        //Prepare data to write by checking if reset is required

        const now = new Date().getTime();
        const trackingData = localData[TRACKING_KEY] || {};
        for (const domain in trackingData) {
          const siteSettings = (localData[UNALLOWED_KEY] || []).find(item => item.url === domain);
          if (!siteSettings) continue;
          const entry = trackingData[domain];
          const lastResetDate = new Date(entry.lastReset);
          let needsReset = false;
          if (siteSettings.type === 'daily' && !isSameDay(new Date(now), lastResetDate)) {
            needsReset = true;
          }
          if (needsReset) {
            console.log(`(Sync) Resetting time for ${domain} (${siteSettings.type})`);
            entry.time = 0;
            entry.lastReset = now;
          }
        }
